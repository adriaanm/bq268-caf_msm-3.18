...
static ssize_t fbtft_fb_read(struct fb_info *info, char __user *buf,
...
static struct fb_ops fbtft_ops = {
	.owner        = THIS_MODULE,
	.fb_read      = fbtft_fb_read,
...
static int fbtft_init_display_from_property(struct fbtft_par *par)
{
	...
	count = of_property_count_u32_elems(dev->of_node, "init");
	if (count < 0)
		return count;
	if (count == 0)
		return -EINVAL;
	...
	ret = of_property_read_u32_array(dev->of_node, "init", values, count);
	if (ret)
		goto out_free;
